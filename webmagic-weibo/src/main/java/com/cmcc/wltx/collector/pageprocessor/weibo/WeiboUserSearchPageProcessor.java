 ...
 *
 */
public class WeiboUserSearchPageProcessor extends BasicPageProcessor {
	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory
			.getLogger(WeiboUserSearchPageProcessor.class);
	private static final String SEARCH_URL_PREFIX = "http://s.weibo.com/user/&nickname=";
	private static final String SEARCH_URL_SUFFIX = "&auth=org_vip";

	public WeiboUserSearchPageProcessor(MySite site) {
		super(site);
	}
	...
	public void process(Page page) throws PageProcessException {
		...
		int index_of_pl_user_feedList_start = rawText.indexOf("<script>STK && STK.pageletM && STK.pageletM.view({\"pid\":\"pl_user_feedList\",");
		if (-1 == index_of_pl_user_feedList_start) {
			throw new PageProcessException("-1 == index_of_pl_user_feedList_start");
		}
		index_of_pl_user_feedList_start += 49;
		int index_of_pl_user_feedList_end = rawText.indexOf(")</script>", index_of_pl_user_feedList_start);
		if (-1 == index_of_pl_user_feedList_end) {
			throw new PageProcessException("-1 == index_of_pl_user_feedList_end");
		}
		String html;
		try {
			JSObject pl_user_feedList = JSUtils.createJSObject(rawText.substring(index_of_pl_user_feedList_start, index_of_pl_user_feedList_end));
			html = pl_user_feedList.getNotNullString("html").trim();
		} catch (JSException e) {
			throw new PageProcessException("JSON结构异常", e);
		}
		Document doc = Jsoup.parse(html);
		Elements nameEles = doc.select("div.pl_personlist > div.list_person > div.person_detail > .person_name > a[uid]");
		if (nameEles.isEmpty()) {
			logger.warn("nameEles.isEmpty()");
			return;
		}
		for (Element nameEle : nameEles) {
			String uid = nameEle.attr("uid").trim();
			if (uid.isEmpty()) {
				logger.warn("uid.isEmpty()");
				continue;
			}
			articlesLogger.info(uid);
		}
	}
	...
	private static void launch(String[] spiderIds) {
		...
		// 创建爬虫
		BasicPageProcessor pageProcessor = new WeiboUserSearchPageProcessor(
				site);
		int proxyType = MySpider.test ? 0 : 1;
		MySpider spider = MySpider.create(pageProcessor, null,
				new QueueScheduler(), proxyType, new MyNullPipeline());

		// 获取搜索关键词
		File keywordsFile = new File(ConstantsHome.USER_DIR
				+ File.separatorChar + "keywords");
		...
		for (String keyword : keywords) {
			try {
				keyword = URLEncoder.encode(URLEncoder.encode(keyword,
			...
			spider.addRequest(new Request(SEARCH_URL_PREFIX + keyword
					+ SEARCH_URL_SUFFIX));
		}
