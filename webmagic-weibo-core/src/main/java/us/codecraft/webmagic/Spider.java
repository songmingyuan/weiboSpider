...
public class Spider implements Runnable, Task {
    ...
    protected final static int STAT_BLOCKED = 3;
    ...
    @Override
    public void run() {
    	switch (checkRunningStat()) {
		case STAT_RUNNING:
			logger.warn("spider is already running!");
			return;
		case STAT_BLOCKED:
			return;
		default:
        ...
        if (onComplete == 1) {
        	onWaitWhenComplete();
		} else if (onComplete == 2) {
			onExitWhenComplete();
		}
    }
    
    protected void onWaitWhenComplete() {
    	if (CollectionUtils.isNotEmpty(spiderListeners)) {
    		for (SpiderListener spiderListener : spiderListeners) {
    			spiderListener.onWaitWhenComplete();
    		}
    	}
    }
    
    protected void onExitWhenComplete() {
    	if (CollectionUtils.isNotEmpty(spiderListeners)) {
    		for (SpiderListener spiderListener : spiderListeners) {
    			spiderListener.onExitWhenComplete();
    		}
    	}
    }
    ...
    private int checkRunningStat() {
        while (true) {
            int statNow = stat.get();
            if (statNow == STAT_RUNNING || statNow == STAT_BLOCKED) {
//                throw new IllegalStateException("Spider is already running!");
    ...
    protected void processRequest(Request request) throws PageProcessException {
        Page page = downloader.download(request, this);
        if (page == null) {
        	onError(request, new PageDownloadException("page download failed"));
    ...
    private void addRequest(Request request) {
        ...
        int pushResult = scheduler.push(request, this);
        if (pushResult != Scheduler.PUSH_SUCCESS) {
            if (CollectionUtils.isNotEmpty(spiderListeners)) {
                for (SpiderListener spiderListener : spiderListeners) {
                    spiderListener.onAddRequestException(pushResult, request);
                }
            }
		}
    }
    ...
    public enum Status {
        Init(0), Running(1), Stopped(2), Blocked(3);
